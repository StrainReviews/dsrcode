...
1. Read the log file: `tail -50 ~/.claude/dsrcode.log 2>/dev/null`
2. If file doesn't exist or is empty: "No log file found at ~/.claude/dsrcode.log. The daemon may not have started yet."
...
5. If user wants more: `tail -N ~/.claude/dsrcode.log` or `cat ~/.claude/dsrcode.log`
6. If user asks to filter: `grep "PATTERN" ~/.claude/dsrcode.log`

## Tips
- For real-time monitoring, suggest: set logLevel to "debug" via `/dsrcode:setup` advanced settings
- To clear the log: `> ~/.claude/dsrcode.log`
- Log file location: `~/.claude/dsrcode.log` (configurable via `logFile` in config)
